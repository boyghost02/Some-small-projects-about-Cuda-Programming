...
__global__ void reduceBlksKernel1(int * in, int * out, int n)
{
  ...
  int i = numElemsBeforeBlk + (threadIdx.x << 1);
...
__global__ void reduceBlksKernel2(int * in, int * out,int n)
{
  ...
  for (int stride = 1; stride < 2 * blockDim.x; stride *= 2)
  {
    ...
    if (threadIdx.x < blockDim.x / stride && i + stride < n) 
    {
      in[i] += in[i + stride];
    }
        ...
        bool useDevice=false, dim3 blockSize=dim3(1), int kernelType=1)
{
	...
	else // Use device
	{
		...
		int * d_in, * d_out;
		dim3 gridSize((int)((n - 1) / 2) / blockSize.x + 1); // TODO: Compute gridSize from n and blockSize
